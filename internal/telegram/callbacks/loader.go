...
func LoadCallbacksHandlers(b *bot.Bot, c *container.AppContainer, ctx context.Context, botUsername string) {
	...
	b.RegisterHandler(bot.HandlerTypeCallbackQueryData, "ask_plan_separator", bot.MatchTypeExact, mychannel.MyPlanAskSeparator(c))
	...
	b.RegisterHandlerMatchFunc(func(update *models.Update) bool {
		return matchMyPlanAskSeparator(ctx, update, c)
	}, mychannel.MyPlanFoundSeparator(c))
...
func matchMyPlanAskCaption(ctx context.Context, update *models.Update, c *container.AppContainer) bool {
	...
	session, _ := c.SessionManager.GetPlainCaptionSession(ctx, userID)
	return session != 0
}

func matchMyPlanAskSeparator(ctx context.Context, update *models.Update, c *container.AppContainer) bool {
	// res, _ := json.Marshal(update)
	// fmt.Println(string(res))
	userID := update.Message.From.ID
	session, _ := c.SessionManager.GetPlainSeparatorSession(ctx, userID)
	return session != 0
}
